...
public class UserController {
	...
	public Mono<ResponseEntity<ResponseMessage>> updateAccountDetails(@RequestBody User user, @PathVariable Long id) {
		...
		boolean updatedAccount;
		try {
			updatedAccount = userService.updateAccount(user, id);
			if (!updatedAccount) {
				log.error("Account update failed! User doesn't exists");
				return Mono.just(ResponseEntity.badRequest().body(userService.getResponse(id, HttpStatus.BAD_REQUEST,
						"Account update failed! User doesn't exists")));
			}
			log.info("Account Successfully Updated!");
			return Mono.just(ResponseEntity.ok()
					.body(userService.getResponse(id, HttpStatus.OK, "Account Successfully Updated!")));
